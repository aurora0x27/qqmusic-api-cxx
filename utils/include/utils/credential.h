...
namespace qqmusic::utils {
    ...
    public:
        explicit CredentialInvalidError(const std::string& message) : std::runtime_error(message) {}
    };
    ...
    public:
        ...
        static Credential from_cookies_dict(const std::map<std::string, std::string>& cookies); // 从 cookies 创建凭据

    private:
        bool starts_with(const std::string& str, const std::string& prefix) const; // 检查字符串是否以特定前缀开头
    };
} // namespace qqmusic::utils
