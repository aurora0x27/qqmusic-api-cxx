...
namespace argparse {
namespace detail {
static inline bool _not_space(int ch) {
...
static inline std::string _ltrim_copy(std::string s, bool (*f)(int) = _not_space) {
    _ltrim(s, f);
    return s;
}
static inline std::string _rtrim_copy(std::string s, bool (*f)(int) = _not_space) {
    _rtrim(s, f);
    return s;
}
static inline std::string _trim_copy(std::string s, bool (*f)(int) = _not_space) {
    _trim(s, f);
    return s;
}
template<typename InputIt>
static inline std::string _join(InputIt begin, InputIt end, const std::string& separator = " ") {
...
public:
    ...
    public:
        Result() {}
        Result(std::string err) noexcept
            : _error(true)
            , _what(err) {}
    ...
    void print_help(size_t count = 0, size_t page = 0) {
            ...
            if (_positional_arguments.empty()) {
                ...
                for (auto& v : _positional_arguments) {
                    if (v.first != Argument::Position::LAST) {
                        ...
                        std::cout << " ["
                                  << detail::_ltrim_copy(_arguments[static_cast<size_t>(v.second)]
                                                             ._names[0],
                                                         [](int c) -> bool {
                ...
                if (it == _positional_arguments.end()) {
                    ...
                    std::cout << " [options...] ["
                              << detail::_ltrim_copy(_arguments[static_cast<size_t>(it->second)]
                                                         ._names[0],
                                                     [](int c) -> bool {
                                                         return c != static_cast<int>('-');
                                                     })
        ...
        for (size_t i = page * count; i < std::min<size_t>(page * count + count, _arguments.size());
             i++) {
    ...
    Result parse(int argc, const char* argv[]) {
        ...
        if (argc > 1) {
            ...
            for (auto& a : _arguments) {
                for (auto& n : a._names) {
                    std::string name = detail::_ltrim_copy(n, [](int c) -> bool {
                        return c != static_cast<int>('-');
                    });
            ...
            for (int argv_index = 1; argv_index < argc; ++argv_index) {
                ...
                } else if (arg_len >= 2 && !detail::_is_number(current_arg)) { // ignores the case if
                    // the arg is just a -
            ...
            if (a._values.size() > 0 && a._values[0][0] == '-') {
                ...
                if (_name_map.find(name) != _name_map.end()) {
                    if (a._position == Argument::Position::LAST) {
                        return Result("Poisitional argument expected at the end, but argument "
    ...
    bool exists(const std::string& name) const {
        std::string n = detail::_ltrim_copy(name, [](int c) -> bool {
            return c != static_cast<int>('-');
        });
...
private:
    Result _begin_argument(const std::string& arg, bool longarg, int position) {
        ...
        if (longarg) {
            ...
            if (equal_pos == 0
                || (equal_pos < 0 && arg_name.length() < arg.length())) { // malformed argument
...
template<>
inline std::vector<std::string> ArgumentParser::Argument::get<std::vector<std::string>>() {
    return _values;
}
